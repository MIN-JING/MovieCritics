        ...
        FirebaseFirestore.getInstance()
            ...
            .addSnapshotListener { snapshot, exception ->
                ...
                if (snapshot != null) {
                    if (snapshot.size() >= 1) {
                        ...
                        Logger.w(
                            "[${this::class.simpleName}]" +
                                "getLiveWatchList task.result.size < 1"
                        )
                    ...
                    Logger.w(
                        "[${this::class.simpleName}]" +
                            "getLiveWatchList snapshot == null"
                    )
                }
            }
        return liveData
            ...
            .addSnapshotListener { snapshot, exception ->
                ...
                if (snapshot != null) {
                    if (snapshot.size() >= 1) {
                        ...
                        Logger.w(
                            "[${this::class.simpleName}]" +
                                "getLiveWatchListByUser task.result.size < 1"
                        )
                    ...
                    Logger.w(
                        "[${this::class.simpleName}]" +
                            "getLiveWatchListByUser snapshot == null"
                    )
                }
            }
        return liveData
    ...
    override suspend fun pushMultiWatchListExpiration(
        ...
        query.get().addOnCompleteListener { task ->
            if (task.isSuccessful) {
                Logger.w(
                    "[${this::class.simpleName}]" +
                        "pushWatchListExpiration get() task.result.size = ${task.result.size()}"
                )
                for (document in task.result) {
                    ...
                    itemRef.document(document.id).update(FIELD_EXPIRATION, expiration)
                        .addOnSuccessListener {
                            ...
                            Logger.w(
                                "pushWatchListExpiration update() Failure," +
                                    "Error = ${it.message}"
                            )
            ...
            FirebaseFirestore.getInstance()
                ...
                .addOnCompleteListener { task ->
                    if (task.isSuccessful) {
                        ...
                        continuation.resume(
                            Result.Fail(MovieApplication.instance.getString(
            ...
            .addSnapshotListener { snapshot, exception ->
                ...
                if (snapshot != null) {
                    if (snapshot.size() >= 1) {
                        ...
                        Logger.w(
                            "[${this::class.simpleName}]" +
                                "getLiveScore task.result.size < 1"
                        )
                ...
                .addOnCompleteListener { task ->
                    if (task.isSuccessful) {
                        if (task.result.size() >= 1) {
                            Logger.w(
                                "[${this::class.simpleName}]" +
                                    "getUserByToken task.result.size >= 1"
                            )
                            Logger.d(task.result.first().id + " => " + task.result.first().data)
            ...
            .addOnCompleteListener { task ->
                if (task.isSuccessful) {
                    if (task.result.size() >= 1) {
                        Logger.w(
                            "[${this::class.simpleName}]" +
                                "getUserById task.result.size >= 1"
                        )
                        Logger.d(task.result.first().id + " => " + task.result.first().data)
                        val item = task.result.first().toObject(User::class.java)
                        continuation.resume(Result.Success(item))
                ...
                .addOnCompleteListener { task ->
                    if (task.isSuccessful) {
                        if (task.result.size() >= 1) {
                            Logger.w(
                                "[${this::class.simpleName}]" +
                                    "getUsersByIdList task.result.size >= 1"
                            )
                            Logger.d(
                                "First element is " +
                                    task.result.first().id + " => " + task.result.first().data
                            )
                ...
                .addOnCompleteListener { task ->
                    if (task.isSuccessful) {
                        if (task.result.size() >= 1) {
                            Logger.w(
                                "[${this::class.simpleName}]" +
                                    "getComments task.result.size >= 1"
                            )
                            Logger.d(task.result.first().id + " => " + task.result.first().data)
            ...
            .addSnapshotListener { snapshot, exception ->
                ...
                if (snapshot != null) {
                    if (snapshot.size() >= 1) {
                        ...
                        Logger.w(
                            "[${this::class.simpleName}]" +
                                "getLiveComments task.result.size < 1"
                        )
                    }
                } else {
                    Logger.w(
                        "[${this::class.simpleName}]" +
                            "getLiveComments snapshot == null"
                    )
                }
            }
        return liveData
            ...
            .addSnapshotListener { snapshot, exception ->
                ...
                if (snapshot != null) {
                    if (snapshot.size() >= 1) {
                        ...
                        Logger.w(
                            "[${this::class.simpleName}]" +
                                "getLiveCommentsExcludeBlocks task.result.size < 1"
                        )
                    }
                } else {
                    Logger.w(
                        "[${this::class.simpleName}]" +
                            "getLiveCommentsExcludeBlocks snapshot == null"
                    )
                }
            }
        return liveData
            ...
            .addSnapshotListener { snapshot, exception ->
                ...
                if (snapshot != null) {
                    if (snapshot.size() >= 1) {
                        ...
                        Logger.w(
                            "[${this::class.simpleName}]" +
                                "getLivePersonalComments task.result.size < 1"
                        )
                    }
                } else {
                    Logger.w(
                        "[${this::class.simpleName}]" +
                            "getLivePersonalComments snapshot == null"
                    )
                }
            }
        return liveData
            ...
            .addSnapshotListener { snapshot, exception ->
                ...
                if (snapshot != null) {
                    if (snapshot.size() >= 1) {
                        ...
                        Logger.w(
                            "[${this::class.simpleName}]" +
                                "getLivePersonalFavorites task.result.size < 1"
                        )
                    }
                } else {
                    Logger.w(
                        "[${this::class.simpleName}]" +
                            "getLivePersonalFavorites snapshot == null"
                    )
                }
            }
        return liveData
        ...
        suspendCoroutine { continuation ->
            ...
            query.get().addOnCompleteListener { task ->
                if (task.isSuccessful) {
                    Logger.w(
                        "[${this::class.simpleName}]" +
                            "removeWatchlistMovie get() task.result.size = ${task.result.size()}"
                    )
                    for (document in task.result) {
                        ...
                        itemRef.document(document.id).delete()
                            .addOnSuccessListener {
                                ...
                                Logger.w(
                                    "removeWatchlistMovie delete() Failure," +
                                        "Error = ${it.message}"
                                )
                                continuation.resume(Result.Error(it))
                            }
